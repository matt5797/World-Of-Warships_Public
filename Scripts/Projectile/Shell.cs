    ...
    public class Shell : ProjectileBase
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (isShoot && shellData != null)
            {
                ...
                if (transform.position.y <= DestroyY * 100)
                {
                    Destroy(gameObject);
                }
                ...
                transform.position += fireRotation * new Vector3(0, (float)(v_y / 100), (float)(v_x / 100));
        ...
        void OnSplash()
        {
            foreach (ParticleSystem splashFactory in splashParticles)
            {
                ...
                particle.transform.position = new Vector3(transform.position.x, 10, transform.position.z);
                particle.Play();
            }
            //Managers.Sound.Play("Bomb", Define.Sound.Effect);
            //OnApplyDamage();
